//
//  FieldDefectListSnapshotStore.swift
//  constructionApp
//
//  離線時還原上次成功載入的缺失列表（依專案、狀態篩選、搜尋關鍵字分檔）。
//

import CryptoKit
import Foundation

enum FieldDefectListSnapshotStore {
    private static let subfolder = "DefectListSnapshots"
    ...
    private static func fileURL(projectId: String, statusFilterKey: String, searchQuery: String) -> URL {
        directoryURL.appendingPathComponent(
            "\(keyHex(projectId: projectId, statusFilterKey: statusFilterKey, searchQuery: searchQuery)).json",
            isDirectory: false
        )
    }
